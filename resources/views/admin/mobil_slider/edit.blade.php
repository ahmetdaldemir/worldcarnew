...
@section('content')
    @php use   \App\Models\MobilSliderLanguage; @endphp
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">Slider Ekle</div>
                <form method="post" action="{{route('admin.admin.mobil_slider.update')}}" enctype="multipart/form-data">
                    <div class="card-body">
                        @csrf
                        <div class="col-md-12">
                            <div class="row">
                                            ...
                                            @foreach($languages as $item)
                                                <div class="tab-pane fade @if($item->id == '1') active show @endif"
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <input class="form-control" name="title[]" id="location-title"  value="{{ MobilSliderLanguage::getSelectLang($id,"title",$item->id)}}"/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="exampleInputPassword1">Seo Başlık</label>
                                                        <input class="form-control" name="meta_title[]" id="location-title" value="{{ MobilSliderLanguage::getSelectLang($id,"meta_title[",$item->id)}}"/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="exampleInputPassword1">Seo Açıklaması</label>
                                                        <input class="form-control" id="meta_description" name="meta_description[]" value="{{ MobilSliderLanguage::getSelectLang($id,"meta_description",$item->id)}}" />
                                                    </div>
                                                </div>
                                            @endforeach
                                        </div>
