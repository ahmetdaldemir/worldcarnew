                                    ...
                                    <div class="form-group">
                                        <select class="form-control" name="category">
                                            <?php foreach($categorys as $item){ ?>
                                                   <option value="{{$item->id}}">{{$item->getLangTitle()}}</option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                        ...
                                        <div class="tab-content" id="myTabContent">
                                            @foreach($languages as $item)
                                                <div class="tab-pane fade @if($item->id == '1') active show @endif" id="{{$item->title}}lang" role="tabpanel" aria-labelledby="{{$item->id}}-tab">
                                                    <div class="form-group">
                                                        <input class="form-control" name="title[]" id="location-title"  placeholder="{{$item->title}} Yazı Adı"/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label>Meta Başlığı</label>
                                                        <input class="form-control" name="meta_title[]" id="location-title"  placeholder="{{$item->meta_title}} Meta Başlıkları"/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label>Meta Açıklaması</label>
                                                        <textarea class="form-control" name="short_description[]" id="location-title"  placeholder="{{$item->short_description}} Meta Açıklaması"></textarea>
                                                    </div>
                                                    <div class="form-group">
