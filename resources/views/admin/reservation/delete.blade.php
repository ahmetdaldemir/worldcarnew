...
@section('content')
    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">Silinen Rezervasyonlar</div>
                        ...
                        <table class="table table-sm table-custom spacing8  table-gray-300 ">
                            ...
                            @foreach($reservation as $item)
                                ...
                                <tr>
                                    <td style="vertical-align: middle" class="text-13">
                                        ...
                                        @if($item->email_send == 1)
                                           <?php
                                            ...
                                            ?>
                                             <div style="background: #013473;text-align: center;color: #fff;" data-toggle="tooltip" data-placement="top" data-original-title="{{$reservationlog}}"> {{$item->id}}</div>
                                        @else
                                            <div>  {{$item->id}}</div>
                                        @endif
                                    ...
                                    <td>
                                        <a href="javascript:;" ng-click="getLogList({{$item->id}})" style="position: relative;top: 10px;"><i class="i-Information"></i></a>
                                    </td>
                                     <td class="text-13  font-weight-bold" style="vertical-align: middle"><a  onclick="return confirm('RGeri al işlemi yapmanız gerekmektedir.?')"
                                            target="_blank"
                                            href="#"><i
                                                class="i-Link"></i>{{$item->pnr}}</a></td>
                                    <td style="vertical-align: middle;{{$item->tdcolor()}}"><span
                                            class="text-13 " style="display: flex"><b>{{$item->private_str()}}</b></span>
                                        <?php if(!is_null($item->comfirm_date)){ ?>
                                    <span style="display: flex;font-weight: 800;color: white;text-indent: 11px;background: #4caf50;" class="text-13 flex">{{date("d-m-Y",strtotime($item->comfirm_date))}}</span>
                                        <?php } ?>
                                    </td>

                                    <td style="vertical-align: middle"><span class="text-13">{{$data->getCarInfoFullNoYear($item->car)}} - {{$item->car}}</span></td>
                                    <td style="vertical-align: middle"><span class="text-13">
                                            <?php if($item->plate != 0 && $item->getPlate->id_car != $item->car){ ?>
                                            <i  data-toggle="tooltip" data-placement="top" data-original-title="Farklı Araç Verildi" style="font-size: 22px;color: #f00;font-weight: 600;" class="i-Danger"></i>
                                            <?php }  ?>
                                            {{$data->getPlateReservation($item->plate)}}</span>
                                    </td>
                                    <td style="vertical-align: middle">
                                        <span class="text-13 text-green-900" style="display: flex;">{{date("d-m-Y",strtotime($item->reservationInformation->checkin ?? null))}} {{$item->reservationInformation->checkin_time ?? null}}</span>
                                        <span class="text-13 text-red-900" style="display: flex">{{date("d-m-Y",strtotime($item->reservationInformation->checkout ?? null))}} {{$item->reservationInformation->checkout_time ?? null}}</span>
                                    </td>
                                    ...
                                    <?php if(isset($item->reservationInformation))  { ?>
                                    <td style="vertical-align: middle" class="text-13">
                                        <?php
                                ...
                                <?php }else{?>
                                     <td></td>
                                    <?php } ?>
                                    <td style="vertical-align: middle"  class="text-13 font-weight-bold text-custom">{{$item->getPaymentMethod()}}</td>
                                    ...
                                    <td style="vertical-align: middle">
                                        <a target="_blank" onclick="return confirm('Rezervasyonu aktif etmek istediğinizden eminmisiniz?')" href="{{route('admin.admin.reservation.returnback',['id'=>$item->id])}}" class="btn">
                                            <img style="width: 24px;" src="{{asset('public/assets/images/return.png')}}" />
                                        </a>
                                        <a onclick="return confirm('Rezervasyonu silmek istediğinizden eminmisiniz?')" href="{{route('admin.admin.reservation.forceDeletereservation',['id'=>$item->id])}}" class="btn">
                                            <img style="width: 24px;" src="{{asset('public/assets/images/delete.png')}}" />
                                        </a>
                                    </td>
                                </tr>
                            @endforeach
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" style="right: 25px; position: absolute;">&times;</button>
                        <h4 class="modal-title"><b style="color:#000">Rezervasyon</b>-<b style="color:#f00">Log</b> Bilgileri</h4>
                    </div>
                    ...
                    <div class="modal-footer">
                        <button style="padding-bottom: 2px;padding-top: 5px;padding-left: 5px;padding-right: 5px;" type="button" class="btn btn-default" data-dismiss="modal">KAPAT</button>
                    </div>
                </div>
            </div>
        </div>
