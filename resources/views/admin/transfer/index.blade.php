...
@section('content')
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">Transferler</div>

                <div class="card-body">
                    <div class="col-md-12">
                        <div class="btn-group" role="group" style="float: right;margin: 10px 0px 10px 0;">
                            <a   class="btn btn-primary" href="{{route('admin.admin.transfer.create')}}"><i class="i-Add"></i> Transfer Ekle</a>
                         </div>
                    </div>
                        ...
                        <table class="table table-sm table-gray-300 ">
                            ...
                            @foreach($transfer as $item)
                                <tr>
                                    ...
                                    <td>{{$item->fullname}}</td>
                                    <td>{{$item->phone}}</td>
                                    <td>{{$item->up_location}} - {{$item->drop_location}}</td>
                                    <td>{{$item->check_in}} / {{$item->check_in_time}}</td>
                                    <td>{{$item->price}}</td>
                                    <td>{{$item->driver}}</td>
                                    <td>{{$item->user->name}}</td>
                                    <td>
                                        <select name="status" class="form-control" onchange="location = this.value;">
                                            <option @if($item->status == 'comfirm') {{'selected'}} @endif value="{{route('admin.admin.transfer.statuschange',['id' => $item->id ,'status' => 'comfirm'])}}">Tamamlandı</option>
                                            <option @if($item->status == 'closed') {{'selected'}} @endif  value="{{route('admin.admin.transfer.statuschange',['id' => $item->id ,'status' => 'closed'])}}"> İptal Edildi</option>
                                            <option @if($item->status == 'waiting') {{'selected'}} @endif value="{{route('admin.admin.transfer.statuschange',['id' => $item->id ,'status' => 'waiting'])}}">Beklemede</option>
                                        </select>
                                    </td>
                                    <td>
                                        <a   href="{{route('admin.admin.transfer.edit',['id'=>$item->id])}}" class="btn btn-success ">
                                            <i class="nav-icon i-Pen-2 "></i>
                                        </a>
                                        <a href="{{route('admin.admin.transfer.delete',['id'=>$item->id])}}" class="btn btn-danger ">
