...
class Search
{
    ...
    public static function calculateEkstra($ekstra,$days)
    {
        $x = 0;
        foreach ($ekstra as $key => $value) {
            $x += (Ekstra::where("input_name", $key)->first()->price * $value[0]) * $days;
        }
        return $x;
    }
    ...
    public static function ekstraProcess($ekstra)
    {
        ...
        foreach ($ekstra as $key => $value) {
            ...
            $x = Ekstra::where("input_name", $key)->where("status", "1")->first();
    ...
    public static function getCustomDateTimeDiff($up_date, $down_date, $up_time, $down_time)
    {
       $dateDifference = static::dayDifference($up_date, $down_date);
       return static::timeCalculate($up_time, $down_time, $dateDifference);
    }
}
